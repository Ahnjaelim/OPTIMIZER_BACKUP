...
function rp_main(){
	...
	drawTimeTablePageSlow();
	drawTimeTablePageFast();
...
//            imgImages: imgDataUrls
    ...
    .then(blob => {
        ...
        const filename = `OPTIMIZER 웹 콘텐츠 최적화 보고서_${year}_${month}_${day}.xlsx`;
...
async function getUrlAndCapture() {
    try {
       ...
       var iframe = document.getElementById('page_preview');
	    iframe.src = url;
    } catch (error) {
...
function allPieChart(data) {
	data = data.data;
	const ctx = document.getElementById('allPieChart').getContext('2d');

	// 기존 차트 인스턴스가 있는지 확인하고 파괴
	if (window.pieChart) {
	...
	for (let i = 0; i < data.length; i++) {
	    labels.push(checkTypeKor(data[i].resource_type));
	    allCntData.push(data[i].all_cnt);
	}
	        ...
	        datasets: [
	            {
	                ...
	                data: allCntData,
	                backgroundColor: [
	                    'rgba(92, 184, 92, 0.7)', // 초록색
	                    'rgba(217, 83, 79, 0.7)', // 빨간색
	                    'rgba(91, 192, 222, 0.7)', // 파란색
	                    'rgba(240, 173, 78, 0.7)' // 주황색
	                ],
	                borderColor: [
	                    'rgba(92, 184, 92, 1)',
	                    'rgba(217, 83, 79, 1)',
...
function selectTimetablePage() {
	var resourceType = document.querySelector('input[name="resource_type3"]:checked').value;
	//var statusType = document.querySelector('input[name="status_type"]:checked').value;
	return new Promise((resolve, reject) => {
        $.ajax({
            type: 'GET',
            url: '/selectTimetablePage',
            data: {
            	resource_status : 1,
            	resource_type : resourceType,
            ...
            success: function(res) {
                resolve(res);
                console.log(res);
                slowPageTable.setData(res.slowData);
                fastPageTable.setData(res.fastData);
...
async function getContentChart() {
    try {
        let response = await $.ajax({
            ...
            url: '/contentChartAll',
            data: {
                resource_type : 99 //웹 콘텐츠
                //resource_tpye : 90 //비정형
            }
        });
...
//	data =data.slice(0, Math.min(10, data.length));
    ...
    for (var i = 0; i < data.length; i++) {
    	console.log(data);
		//data[i].ratio = parseInt((data[i].resource_org_size-data[i].greatest_type)/data[i].resource_org_size* 100);
    ...
    if (statusType < 0) {
      ...
      return [
    	  {title:"No", field:"no" ,hozAlign:"center",},
	        ...
	        {title:"기존 렌더링 시간", field:"org_time", width:200,hozAlign:"right",
	        	formatter: function(cell, formatterParams, onRendered) {
					let result = "";
					if(cell.getValue()==0){
						result = UNCHECKED;
	        ...
	        {title:"최적화 렌더링 시간", field:"new_time", width:200,hozAlign:"right",
	        	formatter: function(cell, formatterParams, onRendered) {
					let result = "";
					if(cell.getValue()==0){
						result = UNCHECKED;
...
//  var statusType = parseInt(document.querySelector('input[name="status_type"]:checked').value, 10);
  ...
  if (statusType < 0) {
    return [
      {title: "No", field: "no",  hozAlign: "center"},
      {title: "웹 페이지 주소", field: "page_url",  hozAlign: "left",
      	},
      {title: "기존 렌더링 시간", field: "org_time",  hozAlign: "right",
      		formatter: function(cell, formatterParams, onRendered) {
					let result = "";
    ...
    return [
  	  {title:"No", field:"no" ,hozAlign:"center",},
	        {title:"웹 페이지 주소", field:"page_url", width:200,hozAlign:"left",},
	        {title:"기존 렌더링 시간", field:"org_time", width:200,hozAlign:"right",
	        	formatter: function(cell, formatterParams, onRendered) {
					let result = "";
					if(cell.getValue()==0){
...
function drawTimeTablePageSlow(){
	
	slowPageTable = new Tabulator("#top10slowPageTable", {
	    //height:"380px",
	    columnDefaults:{
	        headerSort:false,
	     //   hozAlign:"center",
...
function drawTimeTablePageFast(){
	fastPageTable = new Tabulator("#top10fastPageTable", {
	   // height:"380px",
	    columnDefaults:{
	        headerSort:false,
	     //   hozAlign:"center",
