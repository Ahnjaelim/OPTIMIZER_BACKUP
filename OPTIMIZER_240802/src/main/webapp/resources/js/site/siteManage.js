	...
	table_resource = new Tabulator("#siteManageTable", {
	    ...
	    ajaxResponse:function(url,prarm,response){
	    	...
	    	for(let i=0;i<response.data.length;i++){
	    		...
	    		response.data[i].detail_btn = `<button class="btn btn-warning-subtle btn-rounded px-1 py-1" style="font-size:12px; color:#CFF250;" onclick="detailSiteBtnEvent(${response.data[i].site_no});"><i class="uil uil-info-circle mr-2" style="color:#CFF250; font-style: normal; margin-right:2px;"></i>상세보기</button>`;
	    		response.data[i].modify_btn = `<button class="btn btn-info-subtle btn-rounded px-1 py-1" style="font-size:12px;" onclick="updateSiteBtnEvent(${response.data[i].site_no});">수정</button>`;
	    		response.data[i].delete_btn = `<button class="btn btn-danger-subtle btn-rounded px-1 py-1" style="font-size:12px;" onclick="deleteSiteBtnEvent(${response.data[i].site_no});"><i class="uil uil-ban mr-2" style="font-style: normal; margin-right:2px;"></i>삭제</button>`;
	    		response.data[i].optimize_btn = `<button class="btn btn-success-subtle btn-rounded px-1 py-1" style="font-size:12px;" onclick="optimizeSiteBtnEvent(${response.data[i].site_no});">최적화</button>`;
...
function deleteSiteBtnEvent(site_no){
	alertify.confirm('사이트 삭제', '해당  사이트를 삭제하시겠습니까 ?',
			function(){ 
		deleteSite(site_no);
		reloadSiteSelect();
		drawSiteTable(); 
...
function optimizeSiteBtnEvent(site_no){
	...
	alertify.confirm('최적화 페이지로 이동', data.site_name +"의  최적화 관리페이지로 이동하시겠습니까?",
			function(){ 
