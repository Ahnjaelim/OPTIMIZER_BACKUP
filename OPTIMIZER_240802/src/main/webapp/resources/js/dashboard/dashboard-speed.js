...
// 애니메이션 설정
const ANIMATION_UNIT_NUMBER = getConfigValue("ANIMATION_UNIT_NUMBER");
...
$(function() {
	...
	// console.log("ANIMATION_UNIT_NUMBER : "+ANIMATION_UNIT_NUMBER);
	...
	if(USE_UNSTRFILE == true){
		...
		$(".switch-btn").addClass("hide");
		$("#cylinder .type05").addClass("hide");
		$("#top-animation .legend .type05").css({"display":"none"});
	}
...
function spinSlotmachine(targetEle, value, int, dcm) {	
	...
	let html = "";
	for(let i = 0; i < 10; i++){
		digitList += `<div class="digit">${i}</div>`;
	}
	for(let i = 1; i <= int; i++){
		html += `<div class="slot-machine" id="slot${i}"><div class="number">${digitList}</div></div>`;
	}
	html += `.`;
	for(let i = 1; i <= dcm; i++){
		html += `<div class="slot-machine" id="slot-fixed${i}"><div class="number-fixed">${digitList}</div></div>`;		
	}
	$(targetEle).html(html);

    const [integerPart, decimalPart] = value.toString().split('.');
    const valueStr = integerPart.padStart(int, '0');
	...
	try {
        ...
        if(result_code == 200){
			...
			$sizeStatus.find(".desc-value").html(diff_size);
        	 ...
        	 $sizeStatus.find(".card-content").css({"opacity": "0.1"});
	         $noDataEle = $sizeStatus.find(".no-data");
			if($noDataEle.length == 0){
				$sizeStatus.find(".card-content").after(NODATA);			
			}
        }
    } catch (error) {
...
async function drawResourceCountEle(){
	...
	try {
		...
		let unoptimizedArray = resourceCountArray.filter(item => item.resource_status == -1 && item.resource_type <= 4);
...
async function drawTimeStatusEle(){
	try {
		...
		if(avgTimeData.result_code==200){
			...
			// console.log(org_time/1000, new_time/1000);
			$timeStatus.find(".value").html(`<strong class="org">${timeUnitFormatter(org_time)}</strong> <i class="fa-solid fa-right-long"></i> <strong class="new slotmachine-container" id="slotmachine2"></strong><strong class="new"><sub>초</sub></strong> <span>${(avgTimeArray.percentage).toFixed(1)}%</span>`);
			const new_time_format = (new_time/1000).toFixed(3);
			spinSlotmachine("#slotmachine2", new_time_format, 3, 3);
			...
			$timeStatus.find(".card-content").css({"opacity":"0.1"});
			$noDataEle = $timeStatus.find(".no-data");
			if($noDataEle.length == 0){
				$timeStatus.find(".card-content").after(NODATA);			
			}
		}
		for(let i = 0; i < avgTimeByTypeArray.length; i++){
			if(avgTimeByTypeArray[i].percentage > 0){
				...
				$tr.find(".time-rate").html(`<div style="width:0%">&nbsp;</div><p>${(avgTimeByTypeArray[i].percentage).toFixed(1)}%</p>`);
		...
		$("#str-predict-comp-rate").html(`<strong style="color:#ffffff;">${(avgTimeArray.percentage).toFixed(1)}%</strong>`);	
...
function selectViewLogAll(){
	...
	$.ajax({
		...
		success: function(response) {
			...
			const interval = ANIMATION_UNIT_NUMBER;
			// console.log("ANIMATION_UNIT_NUMBER", ANIMATION_UNIT_NUMBER);
			let summarizedData = [];
			...
			if (total === 0) {
				...
				/*
				const keepCount = 5; // 남기고 싶은 li 요소의 갯수를 설정하세요
				$(`#cylinder .type01 li`).slice(keepCount).remove();				
				$(`#cylinder .type02 li`).slice(keepCount).remove();				
				$(`#cylinder .type03 li`).slice(keepCount).remove();				
				$(`#cylinder .type04 li`).slice(keepCount).remove();				
				$(`#cylinder .type05 li`).slice(keepCount).remove();
				*/
				
				const keepCount = 5; // 남기고 싶은 li 요소의 갯수를 설정하세요
				const delay = 100; // 각 요소를 제거할 때 딜레이를 설정하세요 (밀리초 단위)
				
				function removeListItems(selector) {
				    const $items = $(selector).slice(keepCount);
				...
				removeListItems('#cylinder .type01 li');
				removeListItems('#cylinder .type02 li');
				removeListItems('#cylinder .type03 li');
				removeListItems('#cylinder .type04 li');
				removeListItems('#cylinder .type05 li');								
			}				
	...
	data.forEach((item, index) => {
		if(item != null && item != undefined){
			...
			setTimeout(function() {
				...
				// console.log("length", length, "interval", interval, "addCount", addCount);
				...
				if((cnt / CYLINDER_UNIT) + 5 >= cylinderCnt){
					...
					let diff = cylinderCnt - (parseInt(cnt/CYLINDER_UNIT)+5);
	        ...
	        <colgroup>
	            ...
	            <thead>
	                ...
	                <tr>
	                    ...
	                    <th class="narrow" style="text-align:right;">원본 용량</th>
	                    ${useUnstrfile != 1 ? '<th class="narrow" style="text-align:right;">원본 렌더링 시간</th>' : ''}
	                </tr>
	            </thead>
	            <tbody>
	        `;
	        for(let i = 0; i < 5; i++){
	            if(i < listData.length){
	                ...
	                html += `<tr>
	                    ...
	                    <td>${icon}</td>
						<td class="narrow">${listData[i].resource_name}</td>
...
function drawTypeSizeStatusEle(){
	
	selectSizeGroupByType()
	.then(sumResourceTypeSizeData => {
