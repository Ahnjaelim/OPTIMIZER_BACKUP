...
const statusArray = [
	{value : 1, label : "최적화 완료", icon : "checkmark-circle", color : "var(--color-blue)"},
	{value : 0, label : "최적화 대기", icon : "pause-circle", color : "var(--color-yellow)"},
	{value : 11, label : "최적화 진행 중", icon : "play-circle", color : "var(--color-green)"},
	{value : -1, label : "최적화 미적용", icon : "remove-circle", color : "var(--color-gray)"},
	{value : 2, label : "최적화 해제", icon : "close-circle", color : "var(--color-red)"},
...
function printResourceType(resource_type){
	result = "";
	const foundItem = typeArray.find(item => item.value === resource_type);
	return `<ion-icon name="${foundItem.icon}"></ion-icon>`;
}