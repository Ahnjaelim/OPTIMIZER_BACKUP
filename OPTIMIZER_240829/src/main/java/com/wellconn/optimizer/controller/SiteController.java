...
public class SiteController {
	...
	/** 안재림 2024.08.13 **/
	@RequestMapping(value ="/selectFirstSite", method = RequestMethod.GET)
	@ResponseBody
	public Map<String,Object> selectFirstSite(HttpServletRequest request,  HttpServletResponse response, SiteManageVO siteManageVO) throws Exception{
		Map<String,Object> result = new HashMap<>();
		// 세션 받기
		HttpSession session = request.getSession();
		UserVO userVo = (UserVO) session.getAttribute("login");
		...
		siteManageVO.setSite_manager(userVo.getLgn_id());
		result.put("data", siteManageService.selectFirstSite(siteManageVO));
		return result ;
	}	
