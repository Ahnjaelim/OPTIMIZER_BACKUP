...
*/    	   
    	   ...
    	   current_predict_cost_1 += ((item.resource_new_size_type1 - item.resource_new_size_type2)*(item.current_month_call_count + item.current_month_opt_count)) * costCtxt.cloud_cost;
...
*/    	    	  
    	...
    	costCtxt.todaySavingCost = current_size1_cost - current_predict_cost_1;
    	...
    	/*console.log(costCtxt.todaySavingCost)*/
	...
	g_DASHBOARD_COST.costDashboardDataBinding = async function(){
    	...
    	if(current_cost_total < previous_cost_total) {
    		costCtxt.diff_cost = 0;
    	}else {
    		 costCtxt.diff_cost = current_cost_total - previous_cost_total; //전월 대비 절감액
    	}

    	/*console.log(current_cost_total, previous_cost_total)*/
/*    	console.log(costCtxt.diff_cost)*/
		...
		costCtxt.todaySavingCost = current_size1_cost - newSavingCost; 
		// 전월 대비 절감액 표시
	    if (costCtxt.diff_cost >= 0) {
...
*/	    costCtxt.lastSavingCost = costCtxt.previous_month_org_size_all - costCtxt.previous_month_opt_size_all // 최적화 후 예상 비용(전월)
	...
	/*	let typeCostData = getDayLogByTopContent();
		const typeArray = typeCostData.data;
		...
		for(let item of typeArray){
			...
			$tr.find(`.traffic-rate div`).css({"width":percentage+"원"});
			$tr.find(`.traffic-rate-value`).text(comma(typeCost.toFixed(0))+"원");*/
		}
	   ...
	   /* sortMode: "remote",*/
	    ...
	    columns: [ 		    	
	    	...
	    	},	    	
	    	{
	    		...
	    		formatter: function(cell, formatterParams, onRendered) {
	    			if(cell.getValue() < 0) {
	    				 return `<span style="color: #FFC107;">${comma(Math.abs(cell.getValue()).toFixed(0))} 감소</span>`;
	    			}else {
