...
/*	$("#side-menu li").not(".menu-title").eq(0).addClass("mm-active");*/
		...
		$("#cloud-submit-btn").click(function(){insertSite()});
		$("#premise-submit-btn").click(function(){insertSite2()});
})
	...
	$.ajax({
		...
		success: function(res) {
			...
			if(res == 1){
				...
				alertify.success('사이트 등록이 완료되었습니다.');	
				insertSsh(url2);
				drawSiteTable(); 
			}else{
				alertify.error('사이트 등록에 실패하였습니다.');						
			}
...
function insertSite2(mode){
	let cloud_no = $("#site_select").val();
	let site_nm = $("#site_nm2").val();
	let site_address = $("#site_address2").val();
	let site_manager = $("#site_manager").val();
	let site_no ="";
...
/*	if(!nullCheck(cloud_no)){
	...
	}*/if($("input[name='server_type2']:checked").val() !== '1'){
		if(!nullCheck($("input[name=ssh_id2]").val())) {
			$("input[name=ssh_id2]").focus();
			alertify.warning('아이디를 입력하세요');
			return;
		}else if(!nullCheck($("input[name=ssh_pw2]").val())) {
			$("input[name=ssh_pw2]").focus();
			alertify.warning('비밀번호를 입력하세요');
			return;
		}else if(!nullCheck($("input[name=ssh_port2]").val())) {
			$("input[name=ssh_port2]").focus();
			alertify.warning('PORT 번호를 입력하세요');
			return;
		}else if(!nullCheck($("input[name=ssh_server_ip2]").val())) {
			$("input[name=ssh_server_ip2]").focus();
			alertify.warning('서버 IP 주소를 입력하세요');
			return;
		}else if(!nullCheck($("input[name=resource_path2]").val())) {
			$("input[name=resource_path2]").focus();
			alertify.warning('웹 컨텐츠 경로를 입력하세요');
			return;
		}
	}
