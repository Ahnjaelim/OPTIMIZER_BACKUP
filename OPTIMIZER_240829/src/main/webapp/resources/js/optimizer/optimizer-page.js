...
pageFnc.updateTabulator = function (){
	...
	if(optimizingUpdateDataByPage.length > 0){ // 현재 페이지에서 최적화 진행 중인 데이터가 있을 때
		...
		html += `<button class="btn btn-sm btn-danger" onclick="pageFnc.deactivateOptimizeAllBtnEvent(this, {page_no : ${global.page_no}, resource_status : 2});"><ion-icon name="close-circle"></ion-icon> 웹 페이지 최적화 해제</button>
		<button class="btn btn-sm btn-primary" onclick="pageFnc.executeOptimizeAllBtnEvent(this, {page_no : ${global.page_no}, resource_status : 0});"><ion-icon name="checkmark-done-circle" role="img" class="md hydrated"></ion-icon> 웹 페이지 최적화</button>`;
	}
...
pageFnc.deactivateOptimizeAllBtnEvent = function(button, param){
	Swal.fire({
		icon: "warning",
		title: "해당 페이지의 모든 웹 콘텐츠를<br />최적화 해제하시겠습니까?",
		text: "",
		showCancelButton: true,
		confirmButtonColor: "#51d28c",
		cancelButtonColor: "#f34e4e",
